        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                if (hit.collider.gameObject.tag=="Entity"&&hit.collider.GetComponent<Entity>().team != actor.team)
                {
                    ...
                    actor.okAttack.transform.position = new Vector3(hit.collider.GetComponent<Entity>().transform.position.x-1, actor.okAttack.transform.position.y+1, hit.collider.GetComponent<Entity>().transform.position.z-1);
